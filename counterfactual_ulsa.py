...
def _tau_coefficient(freq_mhz: jnp.ndarray, electron_temperature_k: float) -> jnp.ndarray:
    return 3.28e-7 * (electron_temperature_k / 1.0e4) ** -1.35 * (freq_mhz * MHZ_TO_GHZ) ** -2.1
...
def _smooth_broken_power_law_scale(
    ...
    """Scale from 408 MHz with asymptotic high- and low-frequency indices."""
    break_ratio = (
        (1.0 + (break_mhz / freq_mhz) ** smoothness)
        / (1.0 + (break_mhz / 408.0) ** smoothness)
    )
    curvature_exponent = (beta_high - beta_low) / smoothness
    return (freq_mhz / 408.0) ** beta_high * break_ratio**curvature_exponent
...
def transfer_one_frequency(
    ...
    differential_em = em_rate * em_scale * ds_kpc
    cumulative_em_edge = jnp.cumsum(differential_em, axis=1)
    cumulative_em_mid = cumulative_em_edge - 0.5 * differential_em
    tau_coeff = _tau_coefficient(freq_mhz, electron_temperature_k)
    tau_mid = tau_coeff * cumulative_em_mid
    tau_total = tau_coeff * cumulative_em_edge[:, -1]
    beta_eff = _frequency_beta(beta + beta_offset, freq_mhz)
    scale = (freq_mhz / 408.0) ** beta_eff
    base = emissivity_408 * emissivity_scale * scale[:, None]
    smooth_synch = jnp.sum(base * jnp.exp(-tau_mid) * ds_kpc, axis=1)
    stochastic_synch = jnp.sum(base * synch_multiplier * jnp.exp(-tau_mid) * ds_kpc, axis=1)
    freefree = electron_temperature_k * (-jnp.expm1(-tau_total))
    extragalactic_unabsorbed = 1.2 * (freq_mhz / 1_000.0) ** -2.58
    # The small CGM/IGM term is applied only to the isotropic background.
    tau_cgm_igm = 0.95 * (freq_mhz / 1.0) ** -2.1
    extragalactic = extragalactic_unabsorbed * jnp.exp(-tau_total - tau_cgm_igm)
    shell_scale = _smooth_broken_power_law_scale(
        freq_mhz,
        shell_beta[:, None, None],
        shell_beta_low[:, None, None],
        shell_spectral_break_mhz,
        shell_spectral_smoothness,
    )
    local_shells = jnp.sum(
        shell_segment_408
        * shell_scale
        * jnp.exp(-tau_coeff * em_scale * shell_foreground_em),
        axis=(0, 1),
    )
    total_without_sources = stochastic_synch + local_shells + freefree + extragalactic
    return total_without_sources, smooth_synch, stochastic_synch, local_shells, freefree, extragalactic
...
def generate(
    ...
    synch_multiplier = shell_multiplier(
        random_fields["synch_shells"], geometry["shell_index"], config.synch_fluctuation_sigma
    )
    em_multiplier = shell_multiplier(
        random_fields["em_shells"], geometry["shell_index"], config.em_fluctuation_sigma
    )
    beta = beta_map(config, geometry, random_fields["beta"])

    emissivity_408, em_rate, anchor_em_rate = physical_fields(
    ...
    shell_segment_408, shell_distance_index = shell_ray_segments(local_catalog, geometry, config)
    cumulative_em_unscaled = np.cumsum(np.asarray(em_rate) * geometry["ds"], axis=1)
    shell_foreground_em = cumulative_em_unscaled[
        np.arange(cumulative_em_unscaled.shape[0])[None, None, :], shell_distance_index
    ]
    shell_beta = jnp.asarray(local_catalog.beta_temperature)
    shell_beta_low = jnp.asarray(local_catalog.low_frequency_beta_temperature)
    shell_segment_408_jax = jnp.asarray(shell_segment_408)
    shell_foreground_em_jax = jnp.asarray(shell_foreground_em)
